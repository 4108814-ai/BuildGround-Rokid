...
# Meetings 1.0.1 keeps the historical Nexus plugin id `assistant` on purpose.
# Nexus Store matches an installed plugin by BOTH pluginId and Android package. Changing only
# pluginId to `meetings` while retaining the same package causes an identity-mismatch drop.
# User-facing identity is still Meetings everywhere.
...
for item in required_manifest:
    if item not in manifest:
        raise SystemExit(f"Missing Meetings 1.0.1 manifest marker: {item}")
required_service = (
    'renderMeetingsControl()',
...
for item in required_service:
    if item not in service:
        raise SystemExit(f"Missing Meetings 1.0.1 service marker: {item}")
